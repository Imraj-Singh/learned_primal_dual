...
def generate_data(validation=False, n_channels=1):
    ...
    for i in range(n_generate):
        if validation:
            phantom = shepp_logan_channels(space, n_channels = n_channels)
        else:
...
with tf.name_scope('tomography'):
    ...
    for i in range(n_iter):
        with tf.variable_scope('dual_iterate_{}'.format(i)):
            ...
            evalop3 = odl_op_layer(primal[..., 3:4])
            update = tf.concat([dual, evalop1, evalop2, evalop3, y_rt], axis=-1)
            # THE NETWORK BELOW
        ...
        with tf.variable_scope('primal_iterate_{}'.format(i)):
            ...
            evalop3 = odl_op_layer_adjoint(dual[..., 2:3])
            update = tf.concat([primal, evalop1, evalop2, evalop3], axis=-1)
            # THE NETWORK BELOW
            update = prelu(apply_conv(update), name='prelu_1')
            update = prelu(apply_conv(update), name='prelu_2')
...
y_arr_validate, x_true_arr_validate = generate_data(validation=True, n_channels=n_channels)
...
for i in range(0, maximum_steps):
    if i%10 == 0:
        y_arr, x_true_arr = generate_data(validation=False, n_channels=n_channels)
